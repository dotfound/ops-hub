...
- **Tasks never live in the body** — they display through the template's task views. Offer `tasks-create` instead.
- **Stamp on write.**
...
8. **Offer the tasks** — derive a proposed task list from the Deliverables (plus the standard bracket tasks) and offer to run `tasks-create`, passing it the brief + this project + the client. Don't create tasks here.
...
- Create the tasks itself (that's `tasks-create`) — it only proposes and hands off.
