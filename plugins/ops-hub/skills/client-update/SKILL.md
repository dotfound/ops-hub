---
name: client-update
description: Use when refreshing an existing client's page body in the Ops Hub, e.g. the user says "update client X", "refresh X", "refresh X's context", "/client-update X", or wants a client's living sections brought current. Rebuilds the body from linked data and connected sources while preserving Manual Notes. Not for creating a new client (that is /client-create), and never changes the fixed properties.
---

# client-update

Refreshes one existing client's page **body** to the same depth `/client-create` first built it, pulling the living sections current from the linked DBs and connected sources. It preserves the never-overwrite section (Manual Notes) verbatim and never touches the fixed properties.

**Core principle:** the body is the living digest; this skill regenerates it on demand. Same body-composition as `/client-create` (one shared spec), minus property writes, plus Manual Notes preservation.
...
4. **Recompose the living sections** per `_shared/client-body.md`: gather from the linked DBs and connected sources, and rebuild every *other* `Area = Client Body` section at full depth. Placeholder any section whose source is missing.
...
6. **Preview.** Show the proposed new body in chat, fenced. State plainly that the never-overwrite section is preserved verbatim, that every other section is regenerated, and that the fixed properties are untouched. Write nothing yet.
7. **On approval, write the body only** (`notion-update-page`, `replace_content`, `allow_deleting_content: true` — client bodies hold no child pages). Revise and re-preview on request; do not write until approved.
8. **Confirm** with the record's URL.

## Hard rules

- **Resolve everything live via shared startup. Never hardcode Notion IDs.**
- **Never touch the fixed properties.** This skill writes the body only. Lifetime Value and all other properties are out of scope; `/client-ltv-sync` maintains LTV.
...
- Change any fixed property, including Lifetime Value.
