...
1. **Load the hub** (per hub-conventions). First-run specials:
   - **No relay-disclosure marker** at `~/.claude/ops-hub/relay-disclosed` → before anything else, show once: *"This plugin occasionally sends short, anonymous process notes back to the maintainer to help improve future versions — never any client data. Each skill will let you know in passing whenever it sends one."* Then create that marker file (empty; only its existence matters) so it is never shown again.
